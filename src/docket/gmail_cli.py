...
from docket.models import ConnectorCheckpoint, GmailSource, ProviderAccount
...
def _status() -> dict[str, object]:
    ...
    with get_session_factory()() as session:
        ...
        source_counts = Counter(session.scalars(select(GmailSource.status)).all())
        claim_expiries = list(
            session.scalars(
                select(GmailSource.claimed_until)
                .where(GmailSource.status == "claimed")
                .order_by(GmailSource.claimed_until)
            )
        )
    aware_expiries = [
