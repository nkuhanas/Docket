...
_SEMANTIC_OPTION_TOKEN_VERSION = 8
_MAC_BYTES = 16
...
def issue_semantic_option_token(
    *,
    ...
    return base64.urlsafe_b64encode(payload + signature[:_NAVIGATION_MAC_BYTES]).rstrip(
        b"="
    ).decode()
...
def decode_semantic_option_token(token: str) -> SemanticOptionReference | None:
    ...
    if (
        ...
        or raw[0] != _SEMANTIC_OPTION_TOKEN_VERSION
    ):
        return None
...
def verify_semantic_option_token(
    ...
    *,
    ...
    return hmac.compare_digest(raw[:-_NAVIGATION_MAC_BYTES], payload) and hmac.compare_digest(
        raw[-_NAVIGATION_MAC_BYTES:], expected_mac
    )
