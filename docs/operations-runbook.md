...
| Daily thread exists but an aggregate card is absent | Inspect the newest projection outbox row and Hermes response; a repeated HTTP 422 `invalid_control` means the plugin rejected Docket's deterministic component set | Pinned plugin control-kind/token-map drift; reproduce the exact Approve/Reject + Review items + proposal-action combination, reload Hermes after the fix, and let the same durable outbox row retry |
| `Page 1 of 1` is already checked and selecting it does nothing | Inspect the projected `review_page` options and confirm every `default` is false; absence of both Hermes interaction logs and a Docket local-action request means Discord emitted no state change | Review paging was rendered like a stateful field instead of an action menu; deploy the unselected-menu fix and refresh the same projection |
