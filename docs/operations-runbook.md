...
| Hermes calls a terminal or time tool before a today/tomorrow Calendar lookup | Inspect the active `docket_list_calendar_events` schema for `relative_day`, then restart Hermes and run `/reload-mcp` | The active session cached the pre-relative-day MCP schema or old manual-intent guidance |
