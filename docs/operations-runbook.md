...
| Calendar card repeats Status/Execution/Effect, uses the event subject as its long title, or dumps a generic Before record | Inspect the state-oriented renderer and rebuild/recreate Docket | Pre-polish image or renderer regression; state belongs in the title/description and updates use one bounded `Delta` with separate Before/After lines per changed property |
